...
function setupMasterSearch() {
    ...
    // Используем debounce для плавного поиска
...
// ====== УСЛУГИ С КНОПКАМИ (горизонтальная прокрутка) ======
function setupServicesScroll() {
    ...
    if (!servicesGrid || !servicesPrevBtn || !servicesNextBtn) return;

    const getServicesStep = () => {
        const firstItem = servicesGrid.querySelector('.service-card');
        if (!firstItem) return 310;
    ...
    const scrollServices = (direction) => {
        servicesGrid.scrollBy({
            left: direction * getServicesStep(),
            behavior: 'smooth'
        });
    };

    servicesPrevBtn.addEventListener('click', () => scrollServices(-1));
    servicesNextBtn.addEventListener('click', () => scrollServices(1));
}
